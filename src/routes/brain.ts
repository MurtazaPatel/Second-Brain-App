...
brainRouter.get('/:shareLink', async (req:CustomRequest,res)=>{
    ...
    const tags =await Promise.all(SharedUserContent.map(async (content)=>{
        const tagIds = content.tags;
        const StringTags = await Promise.all(tagIds.map(async (tagId)=>{
            const tagObj = await tagModel.findById(tagId);
            return tagObj?.title as string;
        }))
        return StringTags;
    }))

    for(let i : number =0; i<SharedUserContent.length;i++){
        SharedUserContent[i].tagTitles  = tags[i] as string[] ;
    }
    console.log(tags);
